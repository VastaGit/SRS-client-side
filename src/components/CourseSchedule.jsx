...
const groupCoursesByDayAndStartTime = courses => {
  ...
  courses.forEach(course => {
    ...
    const key = `${day}-${startTime}-${endTime}-${courseCode}`
    if (!groupedCourses[key]) {
      groupedCourses[key] = []
    }
    groupedCourses[key].push(course)
          ...
          {timeSlots.map((timeSlot, rowIndex) => (
            <tr key={rowIndex}>
              ...
              {daysOfWeek.map((day, dayIndex) => (
                <td key={dayIndex}>
                  {Object.keys(groupedCourses).map((key, groupIndex) => {
                    const [courseDay, courseStartTime, courseEndTime, courseCode] = key.split('-')
                    const courseStartHour = parseInt(courseStartTime.split(':')[0], 10)
                    const courseEndHour = parseInt(courseEndTime.split(':')[0], 10)
                    const isCourseOnThisDay = courseDay === day

                    return isCourseOnThisDay && rowIndex + 8 >= courseStartHour && rowIndex + 8 < courseEndHour ? (
                      <div key={groupIndex} className='course-group'>
                        {groupedCourses[key].map((course, courseIndex) => (
                          <div
                            key={courseIndex}
